...
class Command(BaseCommand):
    def handle(self, *argus, **options):
        print("Deleting All Datas....")
        
        
        Author.objects.all().delete()
        Cateagory.objects.all().delete()
        Post.objects.all().delete()
        
        file = open(settings.BASE_DIR / "new_posts.csv",encoding="utf8")
        ...
        for row in csv_reader:
            ...
            try:
                image_filename = wget.downlaod(image,file_path)
                uploaded_file_url = f'posts/{file_name}'
            except:
                uploaded_file_url = f'posts/system.png'
            ...
            for tag in tags_list:
                if not tag.strip() == "":
                    item, created = Cateagory.objects.get_or_create(title=tag)
